  ...
  return (
    <div className="w-full h-[23rem] py-20 text-white">
      <div onMouseEnter={()=>{mover(index)}} className="max-w-screen-xl mx-auto flex items-center justify-between">
        ...
        <div className="dets w-1/4 flex flex-col items-start">
            <p className="mb-10">{elem.desc}</p>
            <div className="flex items-center justify-center gap-5">{elem.live && <Button />}
            {elem.case && <Button title="Case Study"/>}</div>
        </div>
      </div>
    </div>
  );
